...
for chr in {1..22];
do
grep -F -f tmp${chr} ukb_imp_chr${chr}_afr.bim > out${chr}.txt #tmp is a file with SNP IDs I want to look at. This makes this process faster since I don't need all variants.
awk '{print $2}' out${chr}.txt |sort|uniq -u > tmp_${chr}.txt #keep only bi-allelic SNPs
grep -F -f tmp_22.txt out22.txt | awk 'length($5)<2'|awk 'OFS="\t"{print $2,$5}' > ref_chr${chr}.txt #retain SNP ID and REFERENCE allele
